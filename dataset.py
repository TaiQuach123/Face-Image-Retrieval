...
def create_train_val_test_dct(paths, img_path = './PR_project/img_align_celeba/', identity_path='./PR_Project_Git/identity_CelebA.txt', num_id=50):
    train_dct = {}
    val_dct = {}
    test_dct = {}
    lst_identities = []
    ...
    for identity in identities:
        ...
        if img_file in paths['train'] and int(id_) <= num_id:
            if id_ in train_dct.keys():
                train_dct[id_].append(os.path.join(img_path, img_file))
            else:
                train_dct[id_] = []
                train_dct[id_].append(os.path.join(img_path, img_file))
        elif img_file in paths['val']:
            if id_ in val_dct.keys():
                val_dct[id_].append(os.path.join(img_path, img_file))
            else:
                val_dct[id_] = []
                val_dct[id_].append(os.path.join(img_path, img_file))
            ...
            if id_ in test_dct.keys():
                test_dct[id_].append(os.path.join(img_path, img_file))
            else:
                test_dct[id_] = []
                test_dct[id_].append(os.path.join(img_path, img_file))
